...
#include "GOAL/Common/Exception.h"
...
class ConsecutiveIdSet {
    static constexpr bool SafetyCheck = true;
public:
    using Index = Int; // EXT[szx][4]: make sure no integer overflow! make it a template parameter?
    using Item = Index;
    ...
    void init(Index capacity, Item minValue = 0) {
        lowerBound = minValue;
        upperBound = minValue + capacity;
        itemNum = 0;
        items.resize(capacity);
        index.resize(capacity, InvalidIndex);
    }
